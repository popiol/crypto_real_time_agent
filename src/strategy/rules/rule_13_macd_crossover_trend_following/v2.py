...
def signal(data: MarketData) -> list[BuySignal | SellSignal]:
    ...
    confirmed by elevated trading volume.
    """
    ...
    for pair, pair_data in data.items():
        ...
        # Check for crossover signals with volume confirmation
        # Buy signal: MACD line crosses above the Signal line AND current volume > Volume MA
        if (macd_prev < signal_prev and macd_current > signal_current and
            current_volume > volume_ma_current):
            signals.append(BuySignal(
                pair=pair,
                timestamp=signal_timestamp,
                price=signal_price,
            ))
        # Sell signal: MACD line crosses below the Signal line AND current volume > Volume MA
