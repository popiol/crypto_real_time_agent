...
# For the `_calculate_ema` function used below, which initializes the first EMA value
...
# In this case, `max(26, 9) = 26`.
# This ensures that `ema_fast`, `ema_slow`, `macd_line`, and `signal_line`
# all have at least 26 elements, which is more than enough for `[-1]` and `[-2]`
# access to detect a crossover.
...
def signal(data: MarketData) -> list[BuySignal | SellSignal]:
    ...
    for pair, pair_data in data.items():
        ...
        # The MACD calculation uses the `last_price` from `Tick` objects.
        # This provides a sufficiently long series of prices for the specified periods.
        ...
        if prev_macd <= prev_signal and current_macd > current_signal:
            signals.append(BuySignal(
                ...
                rule_id="macd_crossover_trend_following_v1",
                confidence=None  # Confidence not specified in the rule idea
            ))
