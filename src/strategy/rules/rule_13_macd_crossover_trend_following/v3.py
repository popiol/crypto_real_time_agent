...
SLOW_PERIOD = 26
SIGNAL_PERIOD = 9
VOLUME_AVG_PERIOD = 20  # New parameter for volume moving average
VOLUME_CONFIRMATION_FACTOR = 0.8 # New factor for relaxed volume confirmation
...
def signal(data: MarketData) -> list[BuySignal | SellSignal]:
    ...
    confirmed by relaxed elevated trading volume (at least 80% of its moving average).
    """
    ...
    for pair, pair_data in data.items():
        ...
        # Buy signal: MACD line crosses above the Signal line AND current volume > (Volume MA * 0.8)
        ...
        # Sell signal: MACD line crosses below the Signal line AND current volume > (Volume MA * 0.8)
