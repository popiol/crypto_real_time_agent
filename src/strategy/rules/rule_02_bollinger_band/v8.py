...
RSI_OVERSOLD = 30
RSI_OVERBOUGHT = 70
VOLUME_MULTIPLIER = 1.5 # e.g., current volume > 1.5 * average_volume
...
# Minimum candles needed for Volume Average: BB_PERIOD (as per pseudocode, using BB period for volume average)
...
def signal(data: MarketData) -> list[BuySignal | SellSignal]:
    """
    Generates Buy/Sell signals based on Bollinger Band with RSI and Volume confirmation.

    A Buy signal is emitted if the price drops below the lower Bollinger Band,
    RSI indicates an oversold condition (e.g., below 30), AND there is a
    significant surge in trading volume.

    A Sell signal is emitted if the price rises above the upper Bollinger Band,
    RSI indicates an overbought condition (e.g., above 70), AND there is a
    significant surge in trading volume.
    ...
    for pair, pair_data in data.items():
        ...
        # Use close prices from warm candles for BB and RSI calculations
        closes = [c.close for c in pair_data.warm]
        # Use volumes from warm candles for average volume calculation
        ...
        # This check is mostly redundant due to MIN_CANDLES_REQUIRED, but good for robustness
        ...
        # Handle cases where RSI could not be calculated (e.g., insufficient data)
        if np.isnan(rsi_value):
            continue

        # --- Volume Confirmation Calculation ---
        # We need the last BB_PERIOD volumes for SMA (as per pseudocode, using BB period for volume average)
        volume_arr_for_avg = np.array(volumes[-BB_PERIOD:], dtype=float)
        
        # This check is mostly redundant due to MIN_CANDLES_REQUIRED, but good for robustness
        if len(volume_arr_for_avg) < BB_PERIOD:
            continue
        
        average_volume = np.mean(volume_arr_for_avg)
        current_volume = volumes[-1] # Volume of the latest warm candle
        ...
        # --- Signal Generation ---
        # BUY signal: Price below lower BB AND RSI oversold AND significant volume surge
        ...
        # SELL signal: Price above upper BB AND RSI overbought AND significant volume surge
