...
def signal(data: MarketData) -> list[BuySignal | SellSignal]:
    """
    Implements the Bollinger Band Extremes with RSI Confirmation trading rule.

    A long signal is generated when Bollinger Bands %B falls below 0.0
    and the 14-period RSI is simultaneously below 30 (oversold condition).

    A short signal is generated when Bollinger Bands %B rises above 1.0
    and the 14-period RSI is simultaneously above 70 (overbought condition).
    """
    ...
    for pair, pair_data in data.items():
        ...
        # The `_calculate_bollinger_percent_b` function returns a series starting
        # from the (BB_PERIOD - 1)th index of the original prices.
        # The `_calculate_rsi` function returns a series starting
        # from the (RSI_PERIOD)th index of the original prices.
        # We need to take the last value of each, which corresponds to the latest available price.
        ...
        if last_bb_percent_b < 0.0 and last_rsi < RSI_OVERSOLD_THRESHOLD:
            signals.append(BuySignal(
                pair=pair,
                timestamp=timestamp,
                price=current_price,
                rule_id="BB_RSI_Extreme_Reversal_V1"
            ))
        elif last_bb_percent_b > 1.0 and last_rsi > RSI_OVERBOUGHT_THRESHOLD:
