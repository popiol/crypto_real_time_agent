...
# Relaxed thresholds for BB_RSI_RelaxedEntries_V2
BB_PERCENT_B_OVERSOLD_THRESHOLD = 0.4 # Adjusted from 0.3 to 0.4
RSI_OVERSOLD_THRESHOLD = 50         # Adjusted from 45 to 50
BB_PERCENT_B_OVERBOUGHT_THRESHOLD = 0.6 # Adjusted from 0.7 to 0.6
RSI_OVERBOUGHT_THRESHOLD = 50        # Adjusted from 55 to 50
...
def signal(data: MarketData) -> list[BuySignal | SellSignal]:
    """
    Implements the Relaxed Bollinger Bands %B and RSI Entry Conditions for Mean Reversion (BB_RSI_RelaxedEntries_V2).

    A buy signal is generated when Bollinger Bands %B falls below 0.4
    AND the 14-period RSI is simultaneously below 50.

    A sell signal is generated when Bollinger Bands %B rises above 0.6
    AND the 14-period RSI is simultaneously above 50.
    """
    ...
    for pair, pair_data in data.items():
        ...
        if last_bb_percent_b < BB_PERCENT_B_OVERSOLD_THRESHOLD and last_rsi < RSI_OVERSOLD_THRESHOLD:
            signals.append(BuySignal(
                pair=pair,
                timestamp=timestamp,
                price=current_price,
                rule_id="BB_RSI_RelaxedEntries_V2" # Updated rule_id
            ))
        elif last_bb_percent_b > BB_PERCENT_B_OVERBOUGHT_THRESHOLD and last_rsi > RSI_OVERBOUGHT_THRESHOLD:
