...
BB_STD_DEV = 2.0
RSI_PERIOD = 14
# Relaxed thresholds for BB_RSI_RelaxedThresholds_V1
...
def signal(data: MarketData) -> list[BuySignal | SellSignal]:
    """
    Implements the Relaxed BB %B and RSI Mean Reversion trading rule (BB_RSI_RelaxedThresholds_V1).

    A buy signal is generated when Bollinger Bands %B falls below 0.2
    AND the 14-period RSI is simultaneously below 40.

    A sell signal is generated when Bollinger Bands %B rises above 0.8
    AND the 14-period RSI is simultaneously above 60.
    """
    ...
    for pair, pair_data in data.items():
        ...
        # The length of %B series is len(prices) - BB_PERIOD + 1
        # The length of RSI series is len(prices) - RSI_PERIOD
        # The last values of both series should align to the last price.
        # The shorter series will determine the effective starting point for aligned comparison.
        # For BB_PERIOD=20, RSI_PERIOD=14:
        # %B starts at index `BB_PERIOD - 1` of prices.
        # RSI starts at index `RSI_PERIOD` of prices.
        # The last value of %B corresponds to `prices[-1]`.
        # The last value of RSI corresponds to `prices[-1]`.
        ...
        # Using the relaxed thresholds for BB_RSI_RelaxedThresholds_V1
        if last_bb_percent_b < 0.2 and last_rsi < RSI_OVERSOLD_THRESHOLD:
            signals.append(BuySignal(
                pair=pair,
                timestamp=timestamp,
                price=current_price,
                rule_id="BB_RSI_RelaxedThresholds_V1"
            ))
        elif last_bb_percent_b > 0.8 and last_rsi > RSI_OVERBOUGHT_THRESHOLD:
