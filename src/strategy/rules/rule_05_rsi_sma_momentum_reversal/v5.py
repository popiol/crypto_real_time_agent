...
RULE_ID = "strict_mean_reversion_squeeze_v1"
...
def calculate_price_z_score(closes: list[float], period: int, current_price: float) -> float | None:
    ...
    if std_dev_closes == 0:
        # If no volatility, Z-score is 0.0 if current_price is at the mean, otherwise infinity.
...
def signal(data: MarketData) -> list[BuySignal | SellSignal]:
    ...
    for pair, pair_data in data.items():
        ...
        # 2. Price Z-Score(20) is below -1.0 (strict undervaluation)
        # 3. Bollinger Band Width(20) is below 0.005 (volatility squeeze)
        buy_condition = (
            (prev_rsi < 30 and current_rsi > 30) and
            (price_z_score_current < -1.0) and
            (current_bb_width < 0.005)
        )
