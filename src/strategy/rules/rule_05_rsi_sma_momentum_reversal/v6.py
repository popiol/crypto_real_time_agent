...
# Minimum candles required for all indicators to produce at least two values for crossover.
...
MIN_CANDLES_FOR_ALL_INDICATORS = max(RSI_PERIOD + 2, ZSCORE_PERIOD, BB_PERIOD)
...
def signal(data: MarketData) -> list[BuySignal | SellSignal]:
    ...
    for pair, pair_data in data.items():
        ...
        # Ensure enough warm candles for all indicators, including prev/current RSI
        ...
        if rsi_values is None or len(rsi_values) < 2: # Need at least two RSI values for crossover
            continue
        current_rsi = rsi_values[-1]
        prev_rsi = rsi_values[-2]

        # Price Z-Score (20) - using the close of the most recent complete candle
        price_z_score_current = calculate_price_z_score(warm_closes, ZSCORE_PERIOD, warm_candles[-1].close)
        ...
        # 1. RSI(14) crosses above 30 from oversold
        # 2. Price Z-Score(20) is below -1.5 (strict undervaluation)
        # 3. Bollinger Band Width(20) is below 0.015 (relaxed volatility squeeze)
        buy_condition = (
            (current_rsi < 30) and # RSI is currently oversold
            (price_z_score_current < -1.5) and # Price is significantly undervalued
            (current_bb_width < 0.015) # Relaxed volatility squeeze condition
        )
        ...
        # 1. RSI(14) crosses below 70 from overbought
        # This is an exit for a long position, not an entry for a short.
        sell_condition = (
