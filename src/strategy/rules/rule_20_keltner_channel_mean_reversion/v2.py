...
# Adjusted Stochastic thresholds as per the rule idea
STOCH_OVERSOLD = 30
STOCH_OVERBOUGHT = 70

# Unique identifier for this trading rule
RULE_ID = "fef07593-8f98-482d-8f46-1ef5332dbc3c"
...
def signal(data: MarketData) -> list[BuySignal | SellSignal]:
    """
    Keltner Channel Mean Reversion with Adjusted Stochastic Thresholds.

    This rule modifies rule_20_keltner_channel_mean_reversion_v1 by adjusting the
    Stochastic Oscillator oversold and overbought thresholds to 30 and 70 respectively.
    It emits a Buy signal when price closes below the lower Keltner Channel and
    Stochastic %K is below 30, and a Sell signal when price closes above the upper
    Keltner Channel and Stochastic %K is above 70.
    """
    ...
    for pair, pair_data in data.items():
        ...
        # Price closes below the lower Keltner Channel AND Stochastic %K is oversold (below 30)
        if current_close < lower_band and current_percent_k < STOCH_OVERSOLD:
            signals.append(BuySignal(
                pair=pair,
                timestamp=current_timestamp,
                price=current_close,
                # rule_id is optional and defaults to ""
            ))
        # Sell Signal condition:
        # Price closes above the upper Keltner Channel AND Stochastic %K is overbought (above 70)
