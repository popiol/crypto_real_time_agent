...
def calculate_bollinger_band_width_percentage(closes: list[float], period: int = 20, num_std_dev: float = 2.0) -> list[float]:
    ...
    for i in range(len(sma_series)):
        ...
        if mid_band > 0:
            bb_width = ((upper_band - lower_band) / mid_band) * 100
...
def signal(data: MarketData) -> list[BuySignal | SellSignal]:
    ...
    for pair, pair_data in data.items():
        ...
        # Keltner channel width is typically a multiple of ATR
        # The Keltner Channel Percentage indicates how far the current price is from the midline
        # as a percentage of the channel width (ATR_MULTIPLIER * ATR_VALUE).
        # We use current_price from hot data for the most up-to-date position relative to the channel.
        if atr_value * KELTNER_ATR_MULTIPLIER <= 0: 
            keltner_channel_percentage = 0.0 
        else:
            keltner_channel_percentage = ((current_price - keltner_midline) / (atr_value * KELTNER_ATR_MULTIPLIER)) * 100
        ...
        # spread_rel is already a percentage (e.g., 0.01 for 0.01%)
        bid_ask_spread_percentage = current_tick.spread_rel

        # --- Apply trading conditions for a BUY signal ---
        # Pseudocode: IF (data.warm.hourly_rsi[-1] < 30 AND data.warm.hourly_keltner_channel_percentage[-1] < -30 AND data.warm.hourly_bollinger_band_width[-1] < 4.0 AND data.hot.bid_ask_spread_percentage[-1] < 0.001): SIGNAL_BUY
        
        # Condition 1: Hourly RSI < 30
        cond_rsi = rsi_value < 30
        
        # Condition 2: Hourly Keltner Channel Percentage < -30
        cond_keltner = keltner_channel_percentage < -30
        
        # Condition 3: Hourly Bollinger Band Width < 4.0 (percentage, e.g., 4.0 means 4%)
        cond_bb_width = bollinger_band_width < 4.0
        ...
        if cond_rsi and cond_keltner and cond_bb_width and cond_spread:
            signals.append(BuySignal(
                ...
                price=current_price,
                rule_id="oversold_reversal_v5", 
                reason="Extreme Oversold Reversal: RSI < 30, Keltner < -30%, BB Width < 4.0%, Spread < 0.001%",
                indicators={
                    "rsi_value": rsi_value,
