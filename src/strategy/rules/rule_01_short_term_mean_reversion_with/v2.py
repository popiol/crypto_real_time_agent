...
def signal(data: MarketData) -> list[BuySignal | SellSignal]:
    ...
    for pair, pair_data in data.items():
        ...
        # Keltner (EMA 20, ATR 20) requires at least 20 warm candles.
        # RSI 14 requires at least 14 + 1 = 15 warm candles.
        # So, 20 warm candles is the overall minimum.
        if len(pair_data.warm) < 20:
            continue 
        # We need at least one hot tick for current price, bid, ask, spread.
        ...
        bid = current_tick.bid_price
        ask = current_tick.ask_price
        
        # --- Calculate Keltner Channel Percentage (20-period EMA for midline, 20-period ATR for band) ---
        ema_20_series = calculate_ema(closes, 20)
        atr_20_series = calculate_atr(highs, lows, closes, 20)
        
        # If indicator series are empty (should be caught by initial data check, but for robustness)
        if not ema_20_series or not atr_20_series:
            continue

        keltner_midline = ema_20_series[-1]
        atr_value = atr_20_series[-1]
        ...
        if atr_value * 2 <= 0:
            ...
            keltner_channel_percentage = ((current_price - keltner_midline) / (atr_value * 2)) * 100

        # --- Calculate Relative Strength Index (RSI) (14 periods) ---
        rsi_14_series = calculate_rsi(closes, 14)
        
        # If RSI series is empty (should be caught by initial data check)
        if not rsi_14_series:
            continue
        ...
        if mid_price > 0:
            bid_ask_spread_percentage = (spread / mid_price) * 100
        else:
            bid_ask_spread_percentage = 0.0 # Avoid division by zero if mid_price is zero

        # --- Apply trading conditions for a BUY signal ---
        # Condition 1: Keltner Channel Percentage below -40
        # Condition 2: RSI below 30
        # Condition 3: Bid-ask spread percentage below 0.02%
        if (keltner_channel_percentage < -40 and 
            rsi_14 < 30 and 
            ...
            signals.append(BuySignal(
                ...
                price=current_price, # As per pseudocode, use current_price for the signal
                rule_id="keltner_rsi_oversold_v2",
                reason="Extreme oversold conditions confirmed by Keltner Channel Percentage < -40 and RSI < 30, with low bid-ask spread."
            ))
