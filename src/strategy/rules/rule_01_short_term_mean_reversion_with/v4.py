...
def signal(data: MarketData) -> list[BuySignal | SellSignal]:
    ...
    # ATR for Keltner: 10
    # BB: 20
    MIN_WARM_CANDLES = max(RSI_PERIOD + 1, EMA_KELTNER_PERIOD, ATR_KELTNER_PERIOD, BB_PERIOD) # Max is 20
    ...
    for pair, pair_data in data.items():
        ...
        current_warm_close = closes[-1] # Last completed hourly candle's close
        ...
        if atr_value * KELTNER_ATR_MULTIPLIER <= 0: 
            keltner_channel_percentage = 0.0 # Treat as neutral if ATR is zero or negative
        else:
            keltner_channel_percentage = ((current_warm_close - keltner_midline) / (atr_value * KELTNER_ATR_MULTIPLIER)) * 100
        ...
        # 1. Hourly RSI(14) >= 20 AND <= 40
        # 2. Hourly Keltner Channel Percentage < -50
        # 3. Hourly Bollinger Band Width < 6.0
        # 4. Bid-Ask Spread Percentage < 0.03

        if (rsi_value >= 20 and 
            rsi_value <= 40 and 
            keltner_channel_percentage < -50 and 
            bollinger_band_width < 6.0 and 
            ...
            signals.append(BuySignal(
                ...
                price=current_price,
                rule_id="fix_oversold_mean_reversion_v1",
                reason="Refined Oversold Mean-Reversion: Broader RSI, Deeper Keltner, Volatility & Spread Filters"
            ))
