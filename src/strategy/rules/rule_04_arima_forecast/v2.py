...
def signal(data: MarketData) -> list[BuySignal | SellSignal]:
    ...
    for pair, pair_data in data.items():
        # Ensure we have enough data for ARIMA and RSI calculations
        # ARIMA needs MIN_CANDLES, RSI needs RSI_PERIOD + 1 candles
        if len(pair_data.warm) < max(MIN_CANDLES, RSI_PERIOD + 1) or not pair_data.hot:
            continue
        ...
        if rsi is None:
            continue  # Should not happen due to initial data check

        # --- Signal Generation ---
        # Buy signal: current price significantly below forecast AND RSI oversold
        if arima_deviation < -ARIMA_DEVIATION_THRESHOLD and rsi < RSI_OVERSOLD_THRESHOLD:
            signals.append(BuySignal(pair=pair, timestamp=ts, price=current_price))
        # Sell signal: current price significantly above forecast AND RSI overbought
        elif arima_deviation > ARIMA_DEVIATION_THRESHOLD and rsi > RSI_OVERBOUGHT_THRESHOLD:
